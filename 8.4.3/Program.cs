...
namespace Exercise
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            showingListContent(showingDividedByThree(numbers));
        ...
        static List<float> showingDividedByThree(float[] tab)
        {
            List<float> dividedByThreeArray = new List<float>();
            for (int i = 0; i < tab.Length; i++)
            {
                if (tab[i] % 3 == 0) dividedByThreeArray.Add(tab[i]);
            }
            return dividedByThreeArray;
        }
